...
public class M1N1 extends JFrame implements ActionListener, KeyListener {
    ...
    ReproducirMusica reproductor = new ReproducirMusica("src/Musica/MusicaMundo1.mp3");

    private boolean pausa = false;
    private boolean silencio = false;
    private int botonX = 920;
    ...
    private BufferedImage[] runningAnimation2; //cambios de variables
    private Hitbox2[] runningAnimationHitboxes2;
    private BufferedImage[] Jumpanimation;
    private BufferedImage[] Jumpanimation2; //cambios de variables 
...
//        reproductor.iniciar();
        //Este metodo sirve para poner pausa con esccaped
    ...
    //Este metodo es para dibujar un cuadrado invisible que va a actuiar como boton, no es la imagen
    ...
    //Este metodo sire para mandar a llamar el nombre de la ventana anterior
    ...
    //ESTE METODO SIRVE PARA IMPRIMIR EL NOMBRE DE USUARIO EN EL NIVEL 1
...
//Convertir el entero a string para poder insertar
            ...
            String Query = "SELECT Puntaje FROM M1N1 WHERE BINARY ID_User=?";
            try (PreparedStatement statement = conexion.ConexionBD().prepareStatement(Query)) {
                ...
                try (ResultSet recordAnterior = statement.executeQuery()) {
                    ...
                    if (recordAnterior.next()) {
                        ...
                        if (ScoreNuevo > ScoreAnterior) {
                            String QueryInsertar = "UPDATE M1N1 SET Puntaje=?, Fecha=?, Tiempo_Restante=? WHERE ID_User=?";
                        ...
                        String QueryInsertar = "INSERT INTO M1N1 (ID_User, Puntaje, Fecha, Tiempo_Restante) VALUES (?,?, ?, ?)";
                        Connection con = conexion.ConexionBD();
                        //statement sirve para preparar la query para interactuar con la base de datos
                        PreparedStatement statementInsertar = con.prepareStatement(QueryInsertar);
                        //Los setstring sirven para poder manipular consultas parametrizadas
    ...
    public void actionPerformed(ActionEvent e) {
        if (!pausa) {
            ...
            if (personajeHitbox.intersects(gemhit[frameIndex2])) {
                ...
                //llamar el metodo de insertar para que se haga la interaccion con la base de datos
                insertar();
    ...
    private void drawCoin(Graphics g) {
        if (frameIndex10 >= 0 && frameIndex2 < coin.length && coin[frameIndex10] != null) {
            coinet[frameIndex10].setX(coinx);
            coinet[frameIndex10].setY(550);
            ...
            if (personajeHitbox.intersects(coinet[frameIndex10])) {
                ...
                g.drawImage(coin[frameIndex10], coinx, 550, 40, 65, null);
            }
    ...
    private void drawCoin2(Graphics g) {
        if (frameIndex11 >= 0 && frameIndex11 < coin2.length && coin2[frameIndex11] != null) {
            ...
            if (personajeHitbox.intersects(coinet2[frameIndex11])) {
                ...
                // Solo se muestra la moneda si el persoanje no a pasado sobre ella
                g.drawImage(coin2[frameIndex11], coinx2, 440, 40, 65, null);
            }
    ...
    private void drawCoin3(Graphics g) {
        if (frameIndex12 >= 0 && frameIndex12 < coin3.length && coin3[frameIndex12] != null) {
            ...
            if (personajeHitbox.intersects(coinet3[frameIndex12])) {
                ...
                // Solo se muestra la moneda si el persoanje no a pasado sobre ella
                g.drawImage(coin3[frameIndex12], coinx3, 295, 40, 65, null);
            }
    ...
    //Este metodo sirve para pintar el boton de pausa, esto solo es visual porque es una imagen
