...
public class M2N2 extends JFrame implements ActionListener, KeyListener {
//JORDAN
    ...
    private int Fantasma = 1100;
    private int Fantasx2 = 1100;
    private Hitbox2[] Fantashit2;
    private BufferedImage[] Fantas2;
    ...
    public M2N2() {
        ...
        try {
            fondoImage = ImageIO.read(new File("src/Fondo/MUNDO22.png")); // ruta del fondo
        ...
        Fantas2 = new BufferedImage[6];
        for (int i = 0; i < Fantas2.length; i++) {
            try {
                Fantas2[i] = ImageIO.read(new File("src/Fantasma/" + (i + 1) + ".png"));
            } catch (IOException ex) {
                ex.printStackTrace();
            }
        }
...
//        reproductor.iniciar();
        ...
        Fantashit2 = new Hitbox2[Fantas2.length];
        for (int i = 0; i < Fantashit2.length; i++) {
            Fantashit2[i] = new Hitbox2(Fantasx2, 430, 77, 77);
        }
    ...
    private void insertar() {
        try {
            ...
            String Query = "SELECT Puntaje FROM M2N2 WHERE BINARY ID_User=?";
            try (PreparedStatement statement = conexion.ConexionBD().prepareStatement(Query)) {
                ...
                try (ResultSet recordAnterior = statement.executeQuery()) {
                    ...
                    if (recordAnterior.next()) {
                        ...
                        if (ScoreNuevo > ScoreAnterior) {
                            String QueryInsertar = "UPDATE M2N2 SET Puntaje=?, Fecha=?, Tiempo_Restante=? WHERE ID_User=?";
                        ...
                        String QueryInsertar = "INSERT INTO M2N2 (ID_User, Puntaje, Fecha, Tiempo_Restante) VALUES (?,?, ?, ?)";
                        Connection con = conexion.ConexionBD();
                        //statement sirve para preparar la query para interactuar con la base de datos
                        PreparedStatement statementInsertar = con.prepareStatement(QueryInsertar);
                        //Los setstring sirven para poder manipular consultas parametrizadas
    ...
    public void actionPerformed(ActionEvent e) {
        if (!pausa) {
            ...
            slimexRelativo = Math.min(Math.max(slimexRelativo, 670), 800);
            if (jumping || falling) {
                frameIndex4++;
            ...
            //Zombie 2        
            Zombie = Math.min(Math.max(Zombie, 1200), 1400);
            if (jumping || falling) {
                frameIndex4++;
                if (frameIndex4 >= Jumpanimation.length) {
                    frameIndex4 = 0;
            ...
            Fantas = Math.min(Math.max(Fantas, 1750), 1910);
            if (jumping || falling) {
                frameIndex4++;
                if (frameIndex4 >= Jumpanimation.length) {
                    frameIndex4 = 0;
            ...
            //Zombie 2        
            Fantasma = Math.min(Math.max(Fantasma, 2320), 2830);
            if (jumping || falling) {
                frameIndex4++;
                if (frameIndex4 >= Jumpanimation.length) {
                    frameIndex4 = 0;
            ...
            // Mover slimexRelativo automáticamente en la dirección correspondiente
            if (movimientoDerecha) {
                Fantasma += velocidadMovimiento; // Mover a la derecha
            } else {
                Fantasma -= velocidadMovimiento; // Mover a la izquierda
            }

            // Actualizar la posición de slimex basada en el desplazamiento del escenario
            Fantasx2 = Fantasma + fondoX;
            ...
            if (personajeHitbox.intersects(obstaculohit) || personajeHitbox.intersects(obstaculohit1) || personajeHitbox.intersects(obstaculohit3) || personajeHitbox.intersects(obstaculohit5) || personajeHitbox.intersects(obstaculohit6) || personajeHitbox.intersects(slime2hit[frameIndex3]) || personajeHitbox.intersects(Zombiehit[frameindex6]) || personajeHitbox.intersects(Fantashit[frameindex7]) || personajeHitbox.intersects(Fantashit2[frameindex8])) {
    ...
    private void drawGem(Graphics g) {
        if (frameIndex2 >= 0 && frameIndex2 < gem.length) {
            ...
            g.drawImage(gem[frameIndex2], gemx, 500, 38, 79, null);
    ...
    private void drawCoin(Graphics g) {
        if (frameindex10 >= 0 && frameindex10 < coin.length && coin[frameindex10] != null) {
            ...
            if (personajeHitbox.intersects(coinet[frameindex10])) {
                ...
                g.drawImage(coin[frameindex10], coinx, 420, 40, 65, null);
            }
    ...
    private void drawCoin2(Graphics g) {
        if (frameindex11 >= 0 && frameindex11 < coin2.length && coin2[frameindex11] != null) {
            ...
            if (personajeHitbox.intersects(coinet2[frameindex11])) {
                ...
                // Solo se muestra la moneda si el persoanje no a pasado sobre ella
                g.drawImage(coin2[frameindex11], coinx2, 130, 40, 65, null);
            }
    ...
    private void drawCoin3(Graphics g) {
        if (frameindex12 >= 0 && frameindex12 < coin3.length && coin3[frameindex12] != null) {
            ...
            if (personajeHitbox.intersects(coinet3[frameindex12])) {
                ...
                // Solo se muestra la moneda si el persoanje no a pasado sobre ella
                g.drawImage(coin3[frameindex12], coinx3, 435, 40, 65, null);
    ...
    private void drawCoin4(Graphics g) {
        if (frameindex13 >= 0 && frameindex13 < coin4.length && coin4[frameindex13] != null) {
            ...
            if (personajeHitbox.intersects(coinet4[frameindex13])) {
                ...
                // Solo se muestra la moneda si el persoanje no a pasado sobre ella
                g.drawImage(coin4[frameindex13], coinx4, 175, 40, 65, null);
    ...
    private void drawExtra(Graphics g) {
        if (frameindex14 >= 0 && frameindex14 < Extra.length && Extra[frameindex14] != null) {
            ...
            Extranet[frameindex14].setY(170);
            ...
            if (personajeHitbox.intersects(Extranet[frameindex14])) {
                ...
                // Solo se muestra la moneda si el persoanje no a pasado sobre ella
                g.drawImage(Extra[frameindex14], Extrax, 170, 68, 80, null);
    ...
    private void drawSlime(Graphics g) {
        if (frameIndex3 >= 0 && frameIndex3 < slime2.length) {
            ...
            g.drawImage(slime2[frameIndex3], slimex, 480, 77, 77, null);

            slime2hit[frameIndex3].setX(slimex);
            slime2hit[frameIndex3].setY(480);
    ...
    private void drawZombie(Graphics g) {
        if (frameindex6 >= 0 && frameindex6 < Zombie2.length) {
            ...
            g.drawImage(Zombie2[frameindex6], Zombiex, 170, 77, 77, null);

            Zombiehit[frameindex6].setX(Zombiex);
            Zombiehit[frameindex6].setY(170);
    ...
    private void drawFantasma(Graphics g) {
        if (frameindex7 >= 0 && frameindex7 < Fantas1.length) {
            ...
            g.drawImage(Fantas1[frameindex7], Fantasx, 160, 77, 77, null);

            Fantashit[frameindex7].setX(Fantasx);
            Fantashit[frameindex7].setY(160);
    ...
    private void drawFantasma2(Graphics g) {
        if (frameindex8 >= 0 && frameindex8 < Fantas2.length) {

            g.drawImage(Fantas2[frameindex8], Fantasx2, 450, 77, 77, null);

            Fantashit2[frameindex8].setX(Fantasx2);
            Fantashit2[frameindex8].setY(450);

            Fantashit2[frameindex8].drawHitbox(g);

        }

        frameindex8++;

        if (frameindex8 >= Fantas2.length) {
            frameindex8 = 0;
        }
